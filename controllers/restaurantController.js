...
exports.getAllRestaurants = async (req, res) => {
  try {
    // Check if master token is present
    const token = req.headers.authorization?.split(' ')[1];
    
    if (!token) {
      return res.status(401).json({ 
        success: false, 
        message: "Access denied. Master token required." 
      });
    }
    ...
    let decoded;
    try {
      decoded = jwt.verify(token, process.env.JWT_SECRET || "master-secret-key-2024");
...
// ==================== DELETE RESTAURANT (Master Admin) ====================
exports.deleteRestaurant = async (req, res) => {
  try {
    ...
    console.log("=== DELETE RESTAURANT (Master Admin) ===");
    console.log("Restaurant Slug:", restaurantSlug);
